...
* @Last Modified time: 2018-05-03 13:13:35
*/
...
// 针对ant design的validator验证规则方法
  ...
  getJsonPower(arry) { // 把菜单数组转换成JSON格式
    let jsonPower = {}
    arry.map((value, index, ary) => {
      jsonPower[value.functionCode] = value['hasPower']
    })
    return jsonPower
  }
  getAryPower(arr, url) { // 获取转换权限数据
    let powerAry = []
    arr.map((value, index, ary) => {
      if (value['url'] === url.toString()) {
        powerAry = value['functionList']
      }
    })
    return powerAry
  }
  headFor(o, cb) { // 转换菜单数据
    ...
    o.forEach(child => {
      cb(child)
      if (child.children) {
        this.headFor(child.children, cb)
      }
    })
  }
  authoControl() { // 权限数据返回
    let res = []
    this.headFor(window.functionPower, item => {
      var temp = {}
      temp.functionList = item.functionList
      temp.url = item.url
      res.push(temp)
    })
    let jsonPower = tooler.getJsonPower(tooler.getAryPower(res, storage.get('stateUrl')))
    return jsonPower
  }
}
